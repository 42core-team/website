...
function isConditionExpr(value: unknown): value is ConditionExpr {
  ...
  if (value.type === "property")
    return isUnitPropertyName(value.property);

  if (value.type === "component_count")
    return typeof value.component === "string";

  if (value.type === "not")
    return isConditionExpr(value["1"]);

  if (value.type === "ternary") {
    return isConditionExpr(value.if)
  ...
  try {
    ...
    const components = source.components.flatMap((component) => {
      ...
      const properties = component.properties.flatMap((property) => {
        if (!isRecord(property) || !isUnitPropertyName(property.name))
          return [];

        if (typeof property.modification !== "number")
          return [];
    ...
    if (components.length !== source.components.length)
      return null;
