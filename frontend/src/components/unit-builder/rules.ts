...
export function getTotalCost(config: ComponentsConfig, componentIds: string[]) {
  const counts = countComponents(componentIds)
...
export function conditionToC(expr: ConditionExpr): string {
  ...
  if (expr.type === 'property') return expr.property

  if (expr.type === 'component_count')
    return `component_count("${expr.component}")`

  if (expr.type === 'not') return `(!${conditionToC(expr['1'])})`
...
function evalCondition(
  ...
  if (expr.type === 'property') return properties[expr.property]
  ...
  if (expr.type === 'sum') return a + b
  if (expr.type === 'subtract') return a - b
  if (expr.type === 'multiply') return a * b
  if (expr.type === 'divide') {
    if (b === 0) throw new Error('division by zero')
    return Math.trunc(a / b)
  }
  if (expr.type === 'min') return Math.min(a, b)
  if (expr.type === 'max') return Math.max(a, b)
  if (expr.type === 'less_than') return a < b ? 1 : 0
  if (expr.type === 'greater_than') return a > b ? 1 : 0
  if (expr.type === 'less_than_or_equal') return a <= b ? 1 : 0
  if (expr.type === 'greater_than_or_equal') return a >= b ? 1 : 0
